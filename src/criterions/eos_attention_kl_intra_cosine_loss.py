...
class EOSAttentionKLIntraCosineLoss(nn.Module):
    def __init__(self, args):
        super(EOSAttentionKLIntraCosineLoss, self).__init__()
        self.args = args
    ...
    def _kl_from_cosines(self, teacher_anchor: torch.Tensor, student_anchor: torch.Tensor):
        teacher_prob = F.softmax(teacher_anchor / self.distiller.temperature, dim=0)
        student_prob = F.softmax(student_anchor / self.distiller.temperature, dim=0)
    ...
    def _pair_kd_loss(
        ...
        student_anchor = F.cosine_similarity(aligned_student_vision, aligned_student_text, dim=-1)

        return self._kl_from_cosines(teacher_anchor.detach(), student_anchor)
